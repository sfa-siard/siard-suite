...
public class QualifiedId {
    ...
    public void parseCatalog(String sCatalogDelimited) throws ParseException {
        setCatalog(SqlLiterals.parseId(sCatalogDelimited));
    }
    ...
    public void parseSchema(String sSchemaDelimited) throws ParseException {
        setSchema(SqlLiterals.parseId(sSchemaDelimited));
    }
    ...
    public void parseName(String sNameDelimited) throws ParseException {
        setName(SqlLiterals.parseId(sNameDelimited));
    }

    public String quote() {
        return SqlLiterals.quoteQualifiedName(getCatalog(), getSchema(), getName());
    }

    public String format() {
        return SqlLiterals.formatQualifiedName(getCatalog(), getSchema(), getName());
    }
        ...
        if (sFormatted != null) {
            ...
            for (int iStart = 0; iStart < sFormatted.length(); ) {
                ...
                if (iEnd < sFormatted.length()) {
                    if (!sFormatted.substring(iEnd)
                                   .startsWith(SqlLiterals.sDOT))
