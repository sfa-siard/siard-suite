...
public class EnhancedSqlBaseVisitor<T> extends SqlBaseVisitor<T> {
    ...
    /**
     @Override public T visitColumnRef(SqlParser.ColumnRefContext ctx)
     {
     return visitValueExpressionPrimary(ctx);
     }
     **/
    ...
    protected void setSchemaName(SqlParser.SchemaNameContext ctx, SchemaId sId) {
        try {
            if (ctx.IDENTIFIER()
                   .size() == 1)
                sId.parseSchema(ctx.IDENTIFIER(0)
                                   .getText());
            else if (ctx.IDENTIFIER()
                        .size() == 2) {
                sId.parseCatalog(ctx.IDENTIFIER(0)
                                    .getText());
                sId.parseSchema(ctx.IDENTIFIER(1)
                                   .getText());
            }
        } catch (ParseException pe) {
            throw new IllegalArgumentException("Error visiting SchemaName!", pe);
        }
    }
    ...
    protected void setQualifiedId(SqlParser.QualifiedIdContext ctx, QualifiedId qId) {
        try {
            if (ctx.IDENTIFIER()
                   .size() == 1)
                qId.parseName(ctx.IDENTIFIER(0)
                                 .getText());
            else if (ctx.IDENTIFIER()
                        .size() == 2) {
                qId.parseSchema(ctx.IDENTIFIER(0)
                                   .getText());
                qId.parseName(ctx.IDENTIFIER(1)
                                 .getText());
            } else if (ctx.IDENTIFIER()
                          .size() == 3) {
                qId.parseCatalog(ctx.IDENTIFIER(0)
                                    .getText());
                qId.parseSchema(ctx.IDENTIFIER(1)
                                   .getText());
                qId.parseName(ctx.IDENTIFIER(2)
                                 .getText());
            }
        } catch (ParseException pe) {
            throw new IllegalArgumentException("Error visiting qualified identifier!", pe);
        }
    }
    ...
    protected void setIdentifier(TerminalNode tn, Identifier id) {
        try {
            ...
            throw new IllegalArgumentException("Error visiting Identifier!", pe);
        }
    }

    protected void addIdentifier(TerminalNode tn, IdList il) {
        try {
            il.parseAdd(tn.getText());
        } catch (ParseException pe) {
            throw new IllegalArgumentException("Error visiting Identifier!", pe);
        }
    }
    ...
    protected void setVariableName(SqlParser.VariableNameContext ctx, ColonId cid) {
        try {
            cid.parse(ctx.getText());
        } catch (ParseException pe) {
            throw new IllegalArgumentException("Error visiting VariableName!", pe);
        }
    }

    protected void setIdChain(SqlParser.IdentifierChainContext ctx, IdChain idc) {
        try {
            for (int i = 0; i < ctx.IDENTIFIER()
                                   .size(); i++)
                idc.parseAdd(ctx.IDENTIFIER(i)
                                .getText());
        } catch (ParseException pe) {
            throw new IllegalArgumentException("Error visiting IdentifierChain!", pe);
        }
    }
    ...
    protected BooleanLiteral getBooleanLiteral(TerminalNode tn) {
        ...
        if (tn.getSymbol()
              .getType() == SqlParser.BOOLEAN_LITERAL) {
            if (BooleanLiteral.TRUE.getKeywords()
                                   .equals(tn.getText()
                                             .toUpperCase()))
                bl = BooleanLiteral.TRUE;
            else if (BooleanLiteral.FALSE.getKeywords()
                                         .equals(tn.getText()
                                                   .toUpperCase()))
                bl = BooleanLiteral.FALSE;
            else if (BooleanLiteral.UNKNOWN.getKeywords()
                                           .equals(tn.getText()
                                                     .toUpperCase()))
