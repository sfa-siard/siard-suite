...
public abstract class SqlLiterals {
    ...
    public static String parseStringLiteral(String sQuoted)
        ...
        if ((sQuoted.length() >= 2) && sQuoted.startsWith("'") && sQuoted.endsWith("'")) {
            ...
            for (int i = 0; i < sQuoted.length(); i++) {
                ...
                if (sQuoted.substring(i, i + 1)
                           .equals(sAPOSTROPHE)) {
                    if ((i < sQuoted.length() - 1) && (sQuoted.substring(i + 1, i + 2)
                                                              .equals(sAPOSTROPHE)))
                        i++; // skip next quote
                    else
    ...
    public static java.sql.Date parseDateLiteral(String sDateLiteral)
        ...
        if (sParsed != null) {
            ...
            try {
                dateParsed = new java.sql.Date(sdfDATE.parse(sParsed)
                                                      .getTime());
            } catch (ParseException pe) {
                throw new ParseException("Date format must conform to yyyy-mm-dd!", pe.getErrorOffset());
            }
        } else
    ...
    public static java.sql.Time parseTimeLiteral(String sTimeLiteral)
        ...
        if (sParsed != null) {
            ...
            try {
                timeParsed = new java.sql.Time(sdfTIME.parse(sParsed)
                                                      .getTime());
                if (sMillis != null)
                    timeParsed.setTime(timeParsed.getTime() + Integer.parseInt(sMillis));
            } catch (ParseException pe) {
                throw new ParseException("Time format must conform to hh:mm:ss.fff!", pe.getErrorOffset());
            }
        } else
    ...
    public static java.sql.Timestamp parseTimestampLiteral(String sTimestampLiteral)
        ...
        if (sParsed != null) {
            ...
            try {
                tsParsed = new java.sql.Timestamp(sdfTIMESTAMP.parse(sParsed)
                                                              .getTime());
                if (sNanos != null)
                    tsParsed.setNanos(Integer.parseInt(sNanos));
            } catch (ParseException pe) {
                throw new ParseException("Timestamp format must conform to yyyy-mm-dd hh-mm-ss.fffffffff!", pe.getErrorOffset());
            }
        } else
    ...
    public static Interval parseInterval(String sIntervalLiteral)
        ...
        if (sParsed != null) {
            ...
            if (sParsed.startsWith(sAPOSTROPHE) && (iEndQuote >= 0)) {
                ...
                if (iTo >= 0) {
                    sStart = sIntervalQualifier.substring(0, iTo)
                                               .trim();
                    sEnd = sIntervalQualifier.substring(iTo + K.TO.getKeyword()
                                                                  .length())
                                             .trim();
                }
                /* detach field widths */
                        ...
                        if (ifStart == IntervalField.YEAR) {
                            ...
                            if (ifEnd != null) {
                                if (ifEnd == IntervalField.MONTH) {
                                    ...
                                    if (iHyphen >= 0) {
                                        sYears = sParsed.substring(0, iHyphen)
                                                        .trim();
