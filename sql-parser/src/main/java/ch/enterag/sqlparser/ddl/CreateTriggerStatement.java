...
public class CreateTriggerStatement
    ...
    private class CtsVisitor extends EnhancedSqlBaseVisitor<CreateTriggerStatement> {
        ...
        public CreateTriggerStatement visitOldOrNewValue(SqlParser.OldOrNewValueContext ctx) {
            ...
            setIdentifier(ctx.correlationName()
    ...
    public IdList getColumnNames() {
        return _ilColumnNames;
    }

    private void setColumnNames(IdList ilColumnNames) {
        _ilColumnNames = ilColumnNames;
    }
    ...
    public String format() {
        ...
        if (getOldOrNewRefs().size() > 0) {
            ...
            for (int i = 0; i < getOldOrNewRefs().size(); i++) {
                sStatement = sStatement + sSP + getOldOrNewRefs().get(i)
                                                                 .getKeywords() + sSP +
                        getCorrelationNames().get(i)
                                             .format();
            }
        }
        if (getForEach() != null)
