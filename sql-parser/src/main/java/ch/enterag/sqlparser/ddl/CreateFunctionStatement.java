...
public class CreateFunctionStatement
    ...
    public boolean isStaticDispath() {
        return _bStaticDispatch;
    }

    public void setStaticDispatch(boolean bStaticDispatch) {
        _bStaticDispatch = bStaticDispatch;
    }
    ...
    protected String formatParameters() {
        ...
        for (int i = 0; i < getParameters().size(); i++) {
            if (i > 0)
                s = s + sCOMMA;
            s = s + sNEW_LINE + sINDENT + getParameters().get(i)
                                                         .format();
        }
