    ...
    private class QsVisitor extends EnhancedSqlBaseVisitor<QuerySpecification> {
        ...
        public QuerySpecification visitSelectList(SqlParser.SelectListContext ctx) {
            if (ctx.ASTERISK() != null)
                ...
                for (int i = 0; i < ctx.selectSublist()
                                       .size(); i++) {
        ...
        public QuerySpecification visitFromClause(SqlParser.FromClauseContext ctx) {
            for (int i = 0; i < ctx.tableReference()
                                   .size(); i++) {
        ...
        public QuerySpecification visitGroupByClause(SqlParser.GroupByClauseContext ctx) {
            ...
            for (int i = 0; i < ctx.groupingElement()
                                   .size(); i++) {
    ...
    private TablePrimary getTablePrimary(SqlStatement sqlstmt, QualifiedId qiTable, TablePrimary tp, String sColumnName) {
        ...
        if ((tp.getTableName()
            ...
            QualifiedId qiName = new QualifiedId(
                    tp.getTableName()
                      .getCatalog() != null ? tp.getTableName()
                                                .getCatalog() : sqlstmt.getDefaultCatalog(),
                    tp.getTableName()
                      .getSchema() != null ? tp.getTableName()
                                               .getSchema() : sqlstmt.getDefaultSchema(),
                    tp.getTableName()
                      .getName());
    ...
    public String format() {
        ...
        if (isAsterisk())
            ...
            for (int i = 0; i < getSelectSublists().size(); i++) {
                ...
                sSpecification = sSpecification + sNEW_LINE + sINDENT + getSelectSublists().get(i)
                                                                                           .format();
            }
        }
        /* FROM */
        ...
        for (int i = 0; i < getTableReferences().size(); i++) {
            if (i > 0)
                sSpecification = sSpecification + sCOMMA;
            sSpecification = sSpecification + sSP + getTableReferences().get(i)
                                                                        .format();
        }
        /* WHERE */
        ...
        if (getGroupingElements().size() > 0) {
            ...
            for (int i = 0; i < getGroupingElements().size(); i++) {
                if (i > 0)
                    sSpecification = sSpecification + sCOMMA;
                sSpecification = sSpecification + sSP + getGroupingElements().get(i)
                                                                             .format();
            }
        }
        /* HAVING */
        ...
        if (getWindowNames().size() > 0) {
            ...
            for (int i = 0; i < getWindowNames().size(); i++) {
                if (i > 0)
                    sSpecification = sSpecification + sCOMMA;
                sSpecification = sSpecification + sSP + getWindowNames().get(i)
                                                                        .format() + sSP +
                        K.AS.getKeyword() + sSP + getWindowSpecifications().get(i)
                                                                           .format();
            }
        }
