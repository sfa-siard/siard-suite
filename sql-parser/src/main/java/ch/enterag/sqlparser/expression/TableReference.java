...
public class TableReference
    ...
    private class TrVisitor extends EnhancedSqlBaseVisitor<TableReference> {
        ...
        public TableReference visitTableReference(SqlParser.TableReferenceContext ctx) {
            if (ctx.CROSS() != null) {
                ...
                if (ctx.joinSpecification()
                       .booleanValueExpression() != null) {
                    setBooleanValueExpression(getSqlFactory().newBooleanValueExpression());
                    getBooleanValueExpression().parse(ctx.joinSpecification()
                                                         .booleanValueExpression());
                } else {
                    for (int i = 0; i < ctx.joinSpecification()
                                           .columnName()
                                           .size(); i++) {
                        Identifier idColumnName = new Identifier();
                        setColumnName(ctx.joinSpecification()
                                         .columnName(i), idColumnName);
                        getJoinColumnNames().add(idColumnName);
                    }
                }
            } else if ((ctx.TABLESAMPLE() != null) && (ctx.numericValueExpression()
                                                          .size() > 0)) {
    ...
    public List<Identifier> getJoinColumnNames() {
        return _listJoinColumnNames;
    }

    private void setJoinColumnNames(List<Identifier> listJoinColumnNames) {
        _listJoinColumnNames = listJoinColumnNames;
    }
