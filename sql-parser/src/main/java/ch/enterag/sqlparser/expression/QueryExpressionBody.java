...
public class QueryExpressionBody
    ...
    private class QebVisitor extends EnhancedSqlBaseVisitor<QueryExpressionBody> {
        ...
        public QueryExpressionBody visitQueryExpressionBody(SqlParser.QueryExpressionBodyContext ctx) {
            if ((ctx.queryOperator() != null) && (ctx.queryExpressionBody()
                                                     .size() == 2)) {
                ...
                if (ctx.correspondingSpecification() != null) {
                    ...
                    for (int i = 0; i < ctx.correspondingSpecification()
                                           .columnName()
                                           .size(); i++) {
                        Identifier idColumnName = new Identifier();
                        setColumnName(ctx.correspondingSpecification()
                                         .columnName(i), idColumnName);
            ...
            } else if (ctx.queryExpressionBody()
                          .size() == 1) {
                setQueryExpressionBody(getSqlFactory().newQueryExpressionBody());
                getQueryExpressionBody().parse(ctx.queryExpressionBody(0));
                ...
                for (int i = 0; i < ctx.tableRowValueExpression()
                                       .size(); i++) {
    ...
    public boolean isCorresponding() {
        return _bCorresponding;
    }

    public void setCorresponding(boolean bCorresponding) {
        _bCorresponding = bCorresponding;
    }
    ...
    public List<Identifier> getCorrespondingColumnNames() {
        return _listCorrespondingColumnNames;
    }

    private void setCorrespondingColumnNames(List<Identifier> listCorrespondingColumnNames) {
        _listCorrespondingColumnNames = listCorrespondingColumnNames;
    }
    ...
    public String formatValues() {
        ...
        for (int i = 0; i < getTableRowValueExpressions().size(); i++) {
            if (i > 0)
                s = s + sCOMMA + sSP;
            s = s + getTableRowValueExpressions().get(i)
                                                 .format();
        }
        s = s + sRIGHT_PAREN;
        return s;
            ...
            if (isCorresponding()) {
                ...
                if (getCorrespondingColumnNames().size() > 0) {
                    ...
                    for (int i = 0; i < getCorrespondingColumnNames().size(); i++) {
                        if (i > 0)
                            s = s + sCOMMA + sSP;
                        s = s + getCorrespondingColumnNames().get(i)
                                                             .format();
                    }
                    s = s + sRIGHT_PAREN;
                }
