...
public class GroupingElement
    ...
    private class GeVisitor extends EnhancedSqlBaseVisitor<GroupingElement> {
        ...
        public GroupingElement visitGroupingElement(SqlParser.GroupingElementContext ctx) {
            if (ctx.ordinaryGroupingSet() != null) {
                for (int i = 0; i < ctx.ordinaryGroupingSet()
                                       .identifierChain()
                                       .size(); i++) {
                    IdChain icGroupingSet = new IdChain();
                    setIdChain(ctx.ordinaryGroupingSet()
                                  .identifierChain(i), icGroupingSet);
    ...
    public List<IdChain> getOrdinaryGroupingSets() {
        return _listOrdinaryGroupingSets;
    }

    private void setOrdinaryGroupingSets(List<IdChain> listOrdinaryGroupingSets) {
        _listOrdinaryGroupingSets = listOrdinaryGroupingSets;
    }
    ...
    public boolean isGroupingSets() {
        return _bGroupingSets;
    }

    public void setGroupingSets(boolean bGroupingSets) {
        _bGroupingSets = bGroupingSets;
    }
    ...
    public String format() {
        ...
        if (isRollup() || isCube()) {
            ...
            for (int i = 0; i < getOrdinaryGroupingSets().size(); i++) {
                if (i > 0)
                    s = s + sCOMMA + sSP;
                s = s + getOrdinaryGroupingSets().get(i)
                                                 .format();
            }
            s = s + sRIGHT_PAREN;
            ...
            for (int i = 0; i < getGroupingElements().size(); i++) {
                if (i > 0)
                    s = s + sCOMMA + sSP;
                s = s + getGroupingElements().get(i)
                                             .format();
            }
            s = s + sRIGHT_PAREN;
        } else {
            if (getOrdinaryGroupingSets().size() == 1)
                s = getOrdinaryGroupingSets().get(0)
                                             .format();
            else {
                s = sLEFT_PAREN;
                for (int i = 0; i < getOrdinaryGroupingSets().size(); i++) {
