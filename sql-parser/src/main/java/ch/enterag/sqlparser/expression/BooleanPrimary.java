...
public class BooleanPrimary
    ...
    private class BpVisitor extends EnhancedSqlBaseVisitor<BooleanPrimary> {
        ...
        public BooleanPrimary visitInCondition(SqlParser.InConditionContext ctx) {
            ...
            if (ctx.queryExpression() != null) {
                ...
                for (int i = 0; i < ctx.rowValueExpression()
                                       .size(); i++) {
        ...
        public BooleanPrimary visitLikeCondition(SqlParser.LikeConditionContext ctx) {
            ...
            if (ctx.ESCAPE() != null) {
                try {
                    setEscapeLetter(SqlLiterals.parseStringLiteral(ctx.CHARACTER_STRING_LITERAL()
                                                                      .getText()));
                } catch (ParseException pe) {
                    throw new IllegalArgumentException("Error visiting escape letter", pe);
                }
            }
            return BooleanPrimary.this;
        }
    ...
    public String getEscapeLetter() {
        return _sEscapeLetter;
    }

    public void setEscapeLetter(String sEscapeLetter) {
        _sEscapeLetter = sEscapeLetter;
    }
            ...
            switch (getBooleanCondition()) {
                ...
                case IN:
                    ...
                    if (getQueryExpression() != null)
                        ...
                        for (int i = 0; i < getRowValueExpressions().size(); i++)
                            s = s + sSP + getRowValueExpressions().get(i)
                                                                  .format();
                    }
                    break;
                case LIKE:
                ...
                case OF:
                    ...
                    for (int i = 0; i < getUdtNames().size(); i++) {
                        if (i > 0)
                            s = s + sCOMMA + sSP;
                        if (getExclusives().get(i)
                                           .booleanValue())
                            s = s + K.ONLY.getKeyword() + sSP;
                        s = s + getUdtNames().get(i)
                                             .format();
                    }
                    s = s + sRIGHT_PAREN;
                    break;
    ...
    private Boolean evaluateNullCondition(NullCondition nc, Object oValue) {
        ...
        if (oValue != null) {
            switch (nc) {
                case IS_NULL:
                    bValue = Boolean.valueOf(oValue == null);
                    break;
                case IS_NOT_NULL:
                    bValue = Boolean.valueOf(oValue != null);
                    break;
