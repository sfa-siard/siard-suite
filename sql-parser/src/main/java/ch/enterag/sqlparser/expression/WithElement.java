...
public class WithElement
    ...
    private class WeVisitor extends EnhancedSqlBaseVisitor<WithElement> {
        ...
        public WithElement visitSearchClause(SqlParser.SearchClauseContext ctx) {
            ...
            for (int i = 0; i < ctx.sortSpecificationList()
                                   .sortSpecification()
                                   .size(); i++) {
                SortSpecification ss = getSqlFactory().newSortSpecification();
                ss.parse(ctx.sortSpecificationList()
                            .sortSpecification(i));
    ...
    public Identifier getSearchColumnName() {
        return _idSearchColumnName;
    }

    public void setSearchColumnName(Identifier idSearchColumnName) {
        _idSearchColumnName = idSearchColumnName;
    }
    ...
    public List<Identifier> getCycleColumnNames() {
        return _listCycleColumnNames;
    }

    private void setCycleColumnNames(List<Identifier> listCycleColumnNames) {
        _listCycleColumnNames = listCycleColumnNames;
    }
        ...
        if (getQueryExpression() != null) {
            ...
            if (getSearchFunction() != null) {
                ...
                for (int i = 0; i < getSortSpecifications().size(); i++) {
                    if (i > 0)
                        s = s + sCOMMA + sSP;
                    s = s + getSortSpecifications().get(i)
                                                   .format();
                }
