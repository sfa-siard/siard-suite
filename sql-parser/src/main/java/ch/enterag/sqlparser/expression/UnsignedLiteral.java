...
public class UnsignedLiteral
    ...
    private class UlVisitor extends EnhancedSqlBaseVisitor<UnsignedLiteral> {
        ...
        public UnsignedLiteral visitGeneralLiteral(SqlParser.GeneralLiteralContext ctx) {
            try {
                if (ctx.CHARACTER_STRING_LITERAL() != null)
                    setCharacterString(SqlLiterals.parseStringLiteral(ctx.CHARACTER_STRING_LITERAL()
                                                                         .getText()));
                else if (ctx.NATIONAL_CHARACTER_STRING_LITERAL() != null)
                    setNationalCharacterString(SqlLiterals.parseNationalStringLiteral(ctx.NATIONAL_CHARACTER_STRING_LITERAL()
                                                                                         .getText()));
                else if (ctx.BIT_STRING_LITERAL() != null)
                    setBitString(SqlLiterals.parseBitStringLiteral(ctx.BIT_STRING_LITERAL()
                                                                      .getText()));
                else if (ctx.BYTE_STRING_LITERAL() != null)
                    setBytes(SqlLiterals.parseBytesLiteral(ctx.BYTE_STRING_LITERAL()
                                                              .getText()));
                else if (ctx.DATE_LITERAL() != null)
                    setDate(SqlLiterals.parseDateLiteral(ctx.DATE_LITERAL()
                                                            .getText()));
                else if (ctx.TIME_LITERAL() != null)
                    setTime(SqlLiterals.parseTimeLiteral(ctx.TIME_LITERAL()
                                                            .getText()));
                else if (ctx.TIMESTAMP_LITERAL() != null)
                    setTimestamp(SqlLiterals.parseTimestampLiteral(ctx.TIMESTAMP_LITERAL()
                                                                      .getText()));
                else if (ctx.intervalLiteral() != null)
                    setInterval(SqlLiterals.parseInterval(ctx.intervalLiteral()
                                                             .getText()));
                else if (ctx.BOOLEAN_LITERAL() != null)
                    setBooleanLiteral(SqlLiterals.parseBooleanLiteral(ctx.BOOLEAN_LITERAL()
                                                                         .getText()));
            } catch (ParseException pe) {
                throw new IllegalArgumentException("Error visiting general literal!", pe);
            }
        ...
        public UnsignedLiteral visitUnsignedNumericLiteral(SqlParser.UnsignedNumericLiteralContext ctx) {
            if (ctx.UNSIGNED_APPROXIMATE() != null) {
                try {
                    setApproximate(SqlLiterals.parseApproximateLiteral(ctx.getText()));
                } catch (ParseException pe) {
                    throw new IllegalArgumentException("Error visiting approximate numeric literal!", pe);
                }
            }
        ...
        public UnsignedLiteral visitExactNumericLiteral(SqlParser.ExactNumericLiteralContext ctx) {
            try {
                setExact(SqlLiterals.parseExactLiteral(ctx.getText()));
            } catch (ParseException pe) {
                throw new IllegalArgumentException("Error visiting exact numeric literal!", pe);
            }
            return UnsignedLiteral.this;
        }
    }
    ...
    public DataType getDataType() {
        ...
        if (getApproximate() != null)
            ...
            int iScale = bd.scale();
            int iPrecision = bd.toPlainString()
                               .length();
            ...
            if (iv.getYears() != 0) {
                ...
                ifEnd = ifStart;
                iPrecision = String.valueOf(iv.getYears())
                                   .length();
                iSecondsDecimals = 0;
                if (iv.getMonths() != 0)
