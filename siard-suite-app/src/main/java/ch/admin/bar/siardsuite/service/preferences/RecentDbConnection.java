...
public class RecentDbConnection {
    ...
    public static RecentDbConnection from(
        ...
        return RecentDbConnection.builder()
                                 .name(name)
                                 .dbmsProduct(connectionData.getDbms()
                                                            .getName())
                                 .connectionOptions(serverBasedProp.flatMap(ServerBasedDbmsConnectionProperties::getOptions)
                                                                   .orElse(""))
                                 .host(serverBasedProp.map(ServerBasedDbmsConnectionProperties::getHost)
                                                      .orElse(""))
                                 .port(serverBasedProp.map(ServerBasedDbmsConnectionProperties::getPort)
                                                      .orElse(""))
                                 .dbName(serverBasedProp.map(ServerBasedDbmsConnectionProperties::getDbName)
                                                        .orElse(""))
                                 .user(serverBasedProp.map(ServerBasedDbmsConnectionProperties::getUser)
                                                      .orElse(""))

                                 .file(fileBasedProp.map(prop -> prop.getFile()
                                                                     .getAbsolutePath())
                                                    .orElse(""))
