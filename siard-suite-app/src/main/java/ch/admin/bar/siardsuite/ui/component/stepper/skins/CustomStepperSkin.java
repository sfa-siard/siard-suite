package ch.admin.bar.siardsuite.ui.component.stepper.skins;
...
public class CustomStepperSkin extends SkinBase<MFXStepper> {
    ...
    public CustomStepperSkin(MFXStepper stepper) {
        ...
        this.stepperBar.alignmentProperty()
                       .bind(stepper.alignmentProperty());
        this.stepperBar.getChildren()
                       .addAll(stepper.getStepperToggles()
                                      .stream()
                                      .filter(Node::isVisible)
                                      .collect(Collectors.toList()));
        this.stepperBar.getStyleClass()
                       .setAll("stepper-bar");
        this.stepperBar.setMaxHeight(52.0);
        this.stepperBar.setMinHeight(52.0);
        layoutWidth(stepper);
        this.stepperBar.setMaxSize(stepper.getPrefWidth(), 52.0);
        this.progressBarGroup.layoutYProperty()
    ...
    private void layoutWidth(MFXStepper stepper) {
        double toggleWidth = stepper.getStepperToggles()
                                    ...
                                    .filter(Node::isVisible)
                                    .mapToDouble(
                                            t -> this.stepperBar.spacingProperty()
                                                                .get() + (TextUtils.computeTextWidth(Font.font("Roboto Regular", 13.0), I18n.get(t.textProperty()
                                                                                                                                                  .get())) + (t.getLabelTextGap() + 22))
                                    )
                                    .sum();
        MFXStepperToggle last = stepper.getStepperToggles()
                                       .stream()
    ...
    private void setListeners() {
        ...
        stepper.getStepperToggles()
               .addListener((ListChangeListener<? super MFXStepperToggle>) (invalidated) -> {
                   stepper.reset();
                   this.stepperBar.getChildren()
                                  .setAll(stepper.getStepperToggles());
                   this.stepperBar.getChildren()
                                  .add(0, this.progressBarGroup);
                   stepper.next();
        ...
        stepper.currentContentProperty()
               .addListener((observable, oldValue, newValue) -> {
                   if (newValue != null) {
                       this.contentPane.getChildren()
                                       .setAll(new Node[]{newValue});
                   } else {
                       this.contentPane.getChildren()
                                       .clear();
                   }
    ...
    private void computeProgress() {
        ...
        if (stepper.isLastToggle()) {
            ...
            if (stepperToggle != null) {
                ...
                if (bounds != null) {
                    double minX = this.snapSize(stepperToggle.localToParent(bounds)
                                                             .getMinX());
                }
            }
        }
    ...
    protected double computeMinWidth(double height, double topInset, double rightInset, double bottomInset, double leftInset) {
        return Math.max(super.computeMinWidth(height, topInset, leftInset, bottomInset, rightInset) + this.getSkinnable()
                                                                                                          .getExtraSpacing() * 2.0, 300.0);
    }
    ...
    protected void printToggleY() {
        this.stepper.getStepperToggles()
                    .forEach(t -> {
