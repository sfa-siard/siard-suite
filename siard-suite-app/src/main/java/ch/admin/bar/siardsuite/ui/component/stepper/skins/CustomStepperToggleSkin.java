...
public class CustomStepperToggleSkin extends SkinBase<MFXStepperToggle> {
    ...
    public CustomStepperToggleSkin(MFXStepperToggle stepperToggle, Boolean visible) {
        ...
        if (visible) {
            ...
            this.container.getStyleClass()
                          .setAll("custom-stepper-toggle");
    ...
    private void setListeners() {
        ...
        this.label.visibleProperty()
                  .bind(stepperToggle.textProperty()
                                     .isEmpty()
                                     .not());
    }
    ...
    protected double computeMinWidth(double height, double topInset, double rightInset, double bottomInset, double leftInset) {
        if (this.visible) {
            return leftInset + this.icon.getMaxWidth() + TextUtils.computeTextWidth(this.label.getFont(), this.label.getText()) + this.getSkinnable()
                                                                                                                                      .getLabelTextGap() * 2.0 + rightInset;
        } else return 0;
    }
    ...
    protected void layoutChildren(double x, double y, double w, double h) {
        ...
        MFXStepperToggle stepperToggle = this.getSkinnable();
        double lw = this.snapSize(w - this.icon.getMaxWidth() - this.getSkinnable()
                                                                    .getLabelTextGap());
        double lh = 52.0;
