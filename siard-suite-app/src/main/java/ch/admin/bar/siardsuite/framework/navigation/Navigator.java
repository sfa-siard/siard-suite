...
public class Navigator {
    ...
    public void navigate(SimpleNavigationTarget target) {
        val nextView = target.getViewSupplier()
                             .apply(servicesFacade);

        log.info("Navigate {}->{} without data",
                 hooksCaller.getPreviouslyLoadedView()
                            .map(loadedView -> loadedView.getController()
                                                         .getClass()
                                                         .getSimpleName())
                            .orElse("n/a"),
                 nextView.getController()
                         .getClass()
                         .getSimpleName());
    ...
    private void navigate(final LoadedView loadedView) {
        viewDisplay.displayView(hooksCaller.nextView(loadedView)
                                           .getNode());
    }
}
