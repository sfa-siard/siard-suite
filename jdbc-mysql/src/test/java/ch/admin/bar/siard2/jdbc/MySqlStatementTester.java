...
public class MySqlStatementTester extends BaseStatementTester {
    ...
    public void testExecute_String_AInt() {
        enter();
        try {
            _stmtMySql.execute(_sSQL_QUERY, new int[]{1});
        } catch (SQLFeatureNotSupportedException sfnse) {
            System.out.println(EU.getExceptionMessage(sfnse));
        } catch (SQLTimeoutException ste) {
            fail(EU.getExceptionMessage(ste));
    ...
    public void testExecute_String_AString() {
        enter();
        try {
            _stmtMySql.execute(_sSQL_QUERY, new String[]{"COL_A"});
        } catch (SQLFeatureNotSupportedException sfnse) {
            System.out.println(EU.getExceptionMessage(sfnse));
        } catch (SQLTimeoutException ste) {
            fail(EU.getExceptionMessage(ste));
    ...
    public void testExecuteQuery() {
        enter();
        try {
            _stmtMySql.executeQuery("SELECT OCTET_LENGTH(TABLE_NAME) FROM INFORMATION_SCHEMA.TABLES");
        } catch (SQLTimeoutException ste) {
            fail(EU.getExceptionMessage(ste));
        } catch (SQLException se) {
            fail(EU.getExceptionMessage(se));
    ...
    public void testExecuteQueryWithUpdate() {
        enter();
        try {
            _stmtMySql.executeUpdate(_sSQL_CLEAN);
        } catch (SQLException se) {
        }
        try {
    ...
    public void testDropTableCascade() {
        enter();
        try {
            _stmtMySql.executeUpdate("DROP TABLE " + TestSqlDatabase.getQualifiedSimpleTable()
                                                                    .format() + " CASCADE");
    ...
    public void testCreateTableLarge() {
        enter();
        try {
            _stmtMySql.executeUpdate("DROP TABLE BUG462 CASCADE");
        } catch (SQLException se) {
            System.out.println(EU.getExceptionMessage(se));
        }
        try {
    ...
    public void testCreateTable() {
        enter();
        try {
            _stmtMySql.executeUpdate("DROP TABLE BUG479 CASCADE");
        } catch (SQLException se) {
            System.out.println(EU.getExceptionMessage(se));
        }
        try {
