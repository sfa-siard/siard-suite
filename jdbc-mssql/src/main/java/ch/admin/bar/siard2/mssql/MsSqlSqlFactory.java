...
import ch.admin.bar.siard2.mssql.datatype.MsSqlDataType;
import ch.admin.bar.siard2.mssql.datatype.MsSqlPredefinedType;
...
public class MsSqlSqlFactory
    ...
    public int getDataType(IdChain icColumn) {
        ...
        if (icColumn.get()
                    .size() > 0) {
            String sColumnName = icColumn.get()
                                         .get(icColumn.get()
                                                      .size() - 1);
            Map<QualifiedId, Integer> mapTableType = _mapColumnType.get(sColumnName);
            if (icColumn.get()
                        .size() > 1) {
                String sTableName = icColumn.get()
                                            .get(icColumn.get()
                                                         .size() - 2);
                for (Iterator<QualifiedId> iterTable = mapTableType.keySet()
                ...
                if (icColumn.get()
                            .size() > 2) {
                    String sSchemaName = icColumn.get()
                                                 .get(icColumn.get()
                                                              .size() - 3);
                    for (Iterator<QualifiedId> iterTable = mapTableType.keySet()
                                                                       .iterator(); iterTable.hasNext(); ) {
            ...
            if (mapTableType.size() == 1) {
                ...
                iDataType = mapTableType.get(qiTable)
                                        .intValue();
            } else
