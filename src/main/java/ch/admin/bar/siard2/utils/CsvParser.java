package ch.admin.bar.siard2.utils;

import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

public class CsvParser implements ch.enterag.utils.csv.CsvParser {
                ...
                if (isNextCharacterEscapedQuote(nextLine, inQuotes(inQuotes), i)) {
                    ...
                    if (!strictQuotes) {
                        if (i > 2 //not on the beginning of the line
                            ...
                            if (ignoreLeadingWhiteSpace && !sb.isEmpty() && isWhitespace(sb)) {
                                sb.setLength(0);
                            } else {
                                sb.append(c);
                            }
    ...
    private boolean shouldConvertEmptyToNull(boolean fromQuotedField) {
        return switch (nullFieldIndicator) {
            case BOTH -> true;
            case EMPTY_SEPARATORS -> !fromQuotedField;
            case EMPTY_QUOTES -> fromQuotedField;
            default -> false;
        };
    }
