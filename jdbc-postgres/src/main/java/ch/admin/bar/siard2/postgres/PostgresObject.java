...
public class PostgresObject {
    ...
    private String getBaseType(String sTypeName) {
        ...
        if (iArray >= 0)
            sTypeName = sTypeName.substring(0, iArray)
                                 .trim();
        int iParen = sTypeName.indexOf("(");
        if (iParen >= 0)
            sTypeName = sTypeName.substring(0, iParen)
                                 .trim();
        return sTypeName;
    }
    ...
    private List<TypeDescription> getAttributeDescriptions()
        ...
        while (rsAttributes.next()) {
            ...
            if ((iDataType == Types.OTHER) && (PostgresType.INTERVAL.getKeyword()
                                                                    .equals(sAttributeTypeName))) {
    ...
    private String fromObject(Object o, int iDataType)
        ...
        switch (iDataType) {
            ...
            case Types.DATE:
                Date date = (Date) o;
                sValue = stripQuotes(PostgresLiterals.formatDateLiteral(date)
                                                     .substring(PostgresLiterals.sDATE_LITERAL_PREFIX.length()));
                break;
            case Types.TIME:
                Time time = (Time) o;
                sValue = stripQuotes(PostgresLiterals.formatTimeLiteral(time)
                                                     .substring(PostgresLiterals.sTIME_LITERAL_PREFIX.length()));
                break;
            case Types.TIMESTAMP:
                Timestamp ts = (Timestamp) o;
                sValue = PostgresLiterals.formatTimestampLiteral(ts)
                                         .substring(PostgresLiterals.sTIMESTAMP_LITERAL_PREFIX.length())
                                         .replace("'", "\"");
                break;
            case Types.OTHER:
                Interval iv = null;
    ...
    private Object fromString(String sValue, int iDataType)
        ...
        switch (iDataType) {
            ...
            case Types.TIMESTAMP:
                ...
                break;
            case Types.OTHER:
                o = PostgresLiterals.parseInterval(stripQuotes(sToken))
                                    .toDuration();
                break;
            case Types.STRUCT:
