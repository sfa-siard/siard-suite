...
public class PostgresResultSetTester
    ...
    private static List<TestColumnDefinition> getListBaseDistinct() {
        ...
        listBaseDistinct.add(new TestColumnDefinition(TestSqlDatabase.getQualifiedDistinctType()
                                                                     .format(), "NCHAR VARYING(10)", "AuwäähNiño"));
    ...
    private static List<TestColumnDefinition> getListCdComplex() {
        ...
        listCdComplex.add(new TestColumnDefinition("COMPLETE", TestSqlDatabase.getQualifiedAllType()
                                                                              .format(), _listCdSimple));
    ...
    public void setUp() {
        try {
            ...
            _conn = dsPostgres.getConnection();
    ...
    public void testGetUnicodeStream() {
        ...
        try {
            ...
            Reader rdr = new InputStreamReader(getResultSet().getUnicodeStream(tcd.getName()), StandardCharsets.UTF_8);
    ...
    public void testGetObject_Map() {
        ...
        try {
            ...
            String sDataType = Types.STRUCT + " (" + SqlTypes.getTypeName(Types.STRUCT) + ")";
...
//    assertEquals(new URL((String) tcd.getValue()), url);
    ...
    private void checkTsQuery(String s, String sExpected) {
        ...
        for (int iToken = 0; iToken < asTokens.length; iToken++) {
            ...
            if (sToken.equals("&") || (sToken.equals("|") || (sToken.equals("!"))))
                ...
                if (iWeight > 0) {
                    sWeight = sToken.substring(iWeight + 1)
                                    .toUpperCase();
    ...
    private void checkBytes(Object o, TestColumnDefinition tcd, String sTypeName, String sDataType)
        ...
        if (o instanceof byte[] buf) {
            ...
            if (tcd.getValue() instanceof UUID uuidExpected) {
                bufExpected = PostgresLiterals.convertUuidToByteArray(uuidExpected);
            } else if (tcd.getValue() instanceof String sExpected) {
                PostgresType pt = PostgresType.getByKeyword(sTypeName);
    ...
    private void checkBigDecimal(Object o, TestColumnDefinition tcd, String sTypeName, String sDataType)
        ...
        if (o instanceof BigDecimal bd) {
            ...
            if (oExpected instanceof BigInteger biExpected) {
    ...
    private void checkOid(long l, byte[] bufExpected)
            throws SQLException {
        PGConnection pgconn = (PGConnection) getResultSet().getStatement()
                                                           .getConnection()
    ...
    private void checkDouble(Object o, TestColumnDefinition tcd, String sTypeName, String sDataType)
        ...
        if (o instanceof Double d) {
            ...
            if (tcd.getValue() instanceof Float f) {
                // truncate double from database
                d = Double.valueOf(d.floatValue());
                // expand float
        ...
        try {
            if (o instanceof Struct struct) {
                if (tcd.getValue() instanceof List<?>) {
                    ...
                    DatabaseMetaData dmd = getResultSet().getStatement()
                                                         .getConnection()
                                                         .getMetaData();
                    ...
                    for (int iAttribute = 0; iAttribute < listAttributes.size(); iAttribute++) {
                        ...
                        if (rsAttribute.next()) {
                            ...
                            String sAttributeDataType = iDataType + " (" + SqlTypes.getTypeName(iDataType) + ")";
                            ...
                            if (tcdAttribute.getName()
                                            .toLowerCase()
                                            .equals(sAttributeName)) {
    ...
    private void checkArray(String sIndent, Object o, TestColumnDefinition tcd, String sTypeName, String sDataType)
        ...
        if (o instanceof Array array) {
            ...
            String sElementDataType = iDataType + " (" + SqlTypes.getTypeName(iDataType) + ")";
            ...
            if (tcd.getValue() instanceof List<?>) {
                ...
                if (ao.length == listElements.size()) {
                    for (int iElement = 0; iElement < listElements.size(); iElement++) {
                        System.out.println(sIndent + tcd.getName() + "[" + iElement + "]: " + sElementDataType + " " + sElementTypeName);
    ...
    private void checkObject(String sIndent, Object o, TestColumnDefinition tcd, int iDataType, String sTypeName, String sDataType)
        ...
        if (tcd.getValue() != null) {
            switch (iDataType) {
                ...
                case Types.NCHAR:
                case Types.NVARCHAR:
                    checkString(o, tcd, sTypeName, sDataType);
                    break;
                case Types.CLOB:
                case Types.NCLOB:
                    checkClob(o, tcd, sTypeName, sDataType);
                    break;
                case Types.SQLXML:
                    checkSqlXml(o, tcd, sTypeName, sDataType);
                    break;
                case Types.BINARY:
                case Types.VARBINARY:
                    checkBytes(o, tcd, sTypeName, sDataType);
                    break;
                case Types.BLOB:
                case Types.DATALINK:
                    checkBlob(o, tcd, sTypeName, sDataType);
                    break;
                case Types.NUMERIC:
                case Types.DECIMAL:
                    checkBigDecimal(o, tcd, sTypeName, sDataType);
                    break;
                case Types.SMALLINT:
                    checkShort(o, tcd, sTypeName, sDataType);
                    break;
                case Types.INTEGER:
                    checkInteger(o, tcd, sTypeName, sDataType);
                    break;
                case Types.BIGINT:
                    checkLong(o, tcd, sTypeName, sDataType);
                    break;
                case Types.DOUBLE:
                    checkDouble(o, tcd, sTypeName, sDataType);
                    break;
                case Types.REAL:
                    checkFloat(o, tcd, sTypeName, sDataType);
                    break;
                case Types.BOOLEAN:
                    checkBoolean(o, tcd, sTypeName, sDataType);
                    break;
                case Types.DATE:
                    checkDate(o, tcd, sTypeName, sDataType);
                    break;
                case Types.TIME:
                    checkTime(o, tcd, sTypeName, sDataType);
                    break;
                case Types.TIMESTAMP:
                    checkTimestamp(o, tcd, sTypeName, sDataType);
                    break;
                case Types.OTHER:
                    checkDuration(o, tcd, sTypeName, sDataType);
                    break;
                case Types.STRUCT:
                    checkStruct(sIndent, o, tcd, sTypeName, sDataType);
                    break;
                case Types.DISTINCT:
                    checkDistinct(o, tcd, sTypeName, sDataType);
                    break;
                case Types.ARRAY:
                    checkArray(sIndent, o, tcd, sTypeName, sDataType);
                    break;
                default:
                    fail("Invalid data type found: " + sDataType + "!");
            }
        } else if (o != null)
    ...
    public void testGetObjectSqlSimple() throws SQLException {
        ...
        for (int iColumn = 0; iColumn < TestSqlDatabase._listCdSimple.size(); iColumn++) {
            ...
            if (rsColumn.next()) {
                ...
                String sDataType = iDataType + " (" + SqlTypes.getTypeName(iDataType) + ")";
    ...
    public void testGetObjectSqlComplex() {
        try {
            ...
            DatabaseMetaData dmd = getResultSet().getStatement()
                                                 .getConnection()
                                                 .getMetaData();
            PostgresQualifiedId pqiTable = new PostgresQualifiedId(TestSqlDatabase.getQualifiedComplexTable()
                                                                                  .format());
    ...
    public void testStruct() throws SQLException {
        ...
        try {
            openResultSet(_sSqlQueryComplex, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
            Connection conn = getResultSet().getStatement()
                                            .getConnection();
            ...
            if (rsColumn.next()) {
                ...
                if (tcd.getName()
                       ...
                       .equals(sColumnName)) {
                    Object o = getResultSet().getObject(sColumnName);
                    if (o instanceof PostgresStruct pstruct) {
    ...
    private boolean equalsStructValue(Struct struct, List<TestColumnDefinition> listAd)
        ...
        if (listAd.size() == struct.getAttributes().length) {
            ...
            for (int iAttribute = 0; iAttribute < listAd.size(); iAttribute++) {
                ...
                } else if (o instanceof SQLXML sqlxml) {
                    o = sqlxml.getString();
                }
                if (o.getClass()
                     .equals(tad.getValue()
    ...
    private Struct createStruct(TestColumnDefinition tcd)
        ...
        for (int i = 0; i < listAttributes.size(); i++) {
            ...
            if (tad.getValue() instanceof List<?>)
                ...
                if (tad.getType()
                       .startsWith(PreType.BLOB.getKeyword())) {
                    Blob blob = conn.createBlob();
                    blob.setBytes(1, (byte[]) tad.getValue());
                    aoAttribute[i] = blob;
                } else if (tad.getType()
                              .startsWith(PreType.CLOB.getKeyword())) {
                    Clob clob = conn.createClob();
                    clob.setString(1, (String) tad.getValue());
                    aoAttribute[i] = clob;
                } else if (tad.getType()
                              .startsWith(PreType.NCLOB.getKeyword())) {
                    NClob nclob = conn.createNClob();
                    nclob.setString(1, (String) tad.getValue());
                    aoAttribute[i] = nclob;
                } else if (tad.getType()
                              .startsWith(PreType.XML.getKeyword())) {
    ...
    private Array createArray(TestColumnDefinition tcd)
        ...
        sBaseTypeName = sBaseTypeName.substring(0, iArray)
                                     .trim();
