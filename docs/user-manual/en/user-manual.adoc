...
The extended version of the user’s manual is only available in English. A short version is also available in French, German, and Italian. Additional information about SIARD Suite is available on the Swiss Federal Archives project website: https://www.bar.admin.ch/bar/en/home/archiving/tools/siard-suite.html
...
In 2015-2018, version 2.1 of the SIARD format was specified by the Swiss Federal Archives in collaboration with the EU project E-ARK and KOST. Like version 1.0, SIARD format 2.1 has been approved as standard eCH-0165 by the association eCH E-Government Standards. In 2021, the DILCIS Board developed and approved the enhanced SIARD Format 2.2. Puzzle ITC has been commissioned by the Swiss Federal Archives to develop SIARD Suite 2.2 according to the enhanced format.
...
- Improved precision of typeOriginal during metadata extraction
- Overloaded functions and packages in Oracle databases are now supported
...
SIARD Suite has been developed by Enter AG and Puzzle ITC AG for the Swiss Federal Archives. The copyright holders release SIARD Suite as Open Source software under the CDDL-1.0 license (in the SIARD distribution as licenses/CDDL-1.0.txt).

SIARD Suite relies on the following third-party components:

* JAVA SE 17 from Oracle https://www.oracle.com/java/technologies/javase/jdk17-archive-downloads.html
** License: Oracle Binary Code License Agreement for the Java SE Platform Products in the SIARD distribution as licenses/java-license.txt

* JavaFX 17.0.7 from OpenJFX https://openjfx.io/
...
* Java Architecture for XML Binding (JAXB) Runtime (Version 2.3.9)
from Eclipse Foundation https://eclipse-ee4j.github.io/jaxb-ri/
** License: Eclipse Distribution License v1.0 in the SIARD distribution as licenses/Eclipse-Distribution-License-1.0.txt
...
* Multi-Schema Validator XSDLib (Version 2013.2.2)
from SUN/Apache https://github.com/kohsuke/msv/
** License: BSD license (BSD-2) in the SIARD distribution as licenses/BSD-2.txt.
...
* Apache Tika Core (Version 2.9.1)
** Content detection and analysis framework from Apache https://tika.apache.org/
** License: Apache License 2.0 in the SIARD distribution as licenses/Apache-license-2.0.txt

* Logback Classic (Version 1.5.18)
** Logging framework from QOS.ch https://logback.qos.ch/
** License: Eclipse Public License v1.0 and the GNU Lesser General Public License version 2.1

* JSON Simple (Version 1.1.1)
** JSON parser from Google Code https://github.com/fangyidong/json-simple
** License: Apache License 2.0 in the SIARD distribution as licenses/Apache-license-2.0.txt

* JetBrains Annotations (Version 24.1.0)
** Annotations for static analysis from JetBrains https://github.com/JetBrains/java-annotations
** License: Apache License 2.0 in the SIARD distribution as licenses/Apache-license-2.0.txt
...
* JDBC Driver for Oracle (ojdbc6.jar, ojdbc7.jar, ojdbc8.jar, xdb6.jar, xmlparserv2.jar) from Oracle
** License: Oracle License in the SIARD distribution as licenses/licenseOracle.txt.

* JdbcMySql from Swiss Federal Archives
** License: CDDL-1.0 license in the SIARD distribution as licenses/CDDL-1.0.txt

* JDBC Driver for MySQL (Version 8.3.0) from Oracle https://dev.mysql.com/downloads/connector/j/
** License: GNU General Public License (GPLv2.0) in the SIARD distribution as licenses/GPLv2.0.txt.

* JdbcMsSql from Swiss Federal Archives
** License: CDDL-1.0 license in the SIARD distribution as licenses/CDDL-1.0.txt

* JDBC Driver for SQL Server (Version 12.2.0) from Microsoft https://learn.microsoft.com/en-us/sql/connect/jdbc/download-microsoft-jdbc-driver-for-sql-server
...
* Jackcess (Version 2.1.12) from Health Market Science http://jackcess.sourceforge.net/
...
A copy of all licenses can be found in the licenses folder of the distribution ZIP file. Third-party dependencies used by SIARD Suite are found in the lib folder of the distribution, or defined in the build.gradle file.
...
7. Define which external documentation must be archived with the SIARD file to ensure comprehensibility of the data (e.g. code tables, system documentation, entity relationship diagram).
...
The SIARD Suite application is a JavaFX application. As such, it can run on any system where a Java Runtime Environment (JRE) with the JavaFX extension is installed.

You can download a JRE 17 with JavaFX support from various sites:

* https://www.azul.com/downloads/?version=java-17-lts&package=jre-fx#zulu
* https://www.oracle.com/java/technologies/downloads/#java17

However, there are distributions of SIARD Suite that already contain the required Java runtime and there are native installers for Windows, Linux and Mac that you can use.
...
In previous versions of SIARD Suite, reliance on ODBC required the use of 32-bit JAVA to access MS Access databases. SIARD Suite 2.2 no longer uses ODBC to access MS Access databases. Therefore, these databases can be accessed on any platform (e.g. LINUX), and SIARD Suite 2.2 is compatible with both 32-bit and 64-bit JAVA. It is recommended to choose the JAVA architecture according to the architecture of your operating system.
...
The distributions with and without JRE are delivered as a ZIP archive. This can be unpacked anywhere. After that the application can be started by one of the following scripts:
...
If the installation of SIARD-Suite is prevented due to security barriers on your computer, please follow the instructions in the link:../../../release-guide.md[release guide].
...
The GUI is the central application that allows users to create SIARD archives and view them later without modifying the primary data of the archive. For complex analysis of the archived data, the archive should be reloaded into a database.

The start screen is the entry point to the application. The four main functionalities of the application can be accessed from here:
...
* Export data from a SIARD archive
...
The location of the data can be recorded during archiving at the "Meta Data" stage.
...
The option to archive views as tables usually leads to redundant storage of the same data and is therefore not recommended. It is useful if the database user used for archiving can see the contents of the views but not the base tables.
...
If the maximum precision and the maximum number of decimal places (e.g. in MS Access) are less than required, the values are uploaded with less precision. SIARD helps itself as much as possible in the target database system.

== Tips and tricks

=== Preparing a database for download

SIARD Suite downloads all data visible to the database user that was used for the connection. The choice of a suitable database user therefore determines the size of the archived database. Often a suitable "technical database user" of a database application is available, which is used to manage the database. This user usually has exactly the access rights needed to archive the database.
...
The conversion of `TIMES` and `TIMESTAMPS` in the database depends on the local time zone. The UTC time 14:30 in the SIARD archive is loaded into the database as local time 15:30 when uploading in Zurich during wintertime. To suppress this conversion, SiardToDb must be started with the option `-Duser.timezone=GMT`, which causes SIARD to interpret all database times as UTC times.

=== Encrypted connections


* *Java 17 Requirements:* For security reasons, recent Java versions no longer support the obsolete TLS 1.0 and 1.1 protocols by default. At least TLS 1.2 is required.
** _Note:_ If TLS 1.0/1.1 is strictly required for legacy systems, it must be explicitly re-enabled in the JVM's `+java.security+` file.

==== MS SQL Server

* *JDBC Driver (Encryption):* Starting with driver version 10.2 (including 13.4.0), encryption is enabled by default (`+encrypt=true+`).
* *Certificate Validation:* The driver now strictly validates whether the server certificate is trusted (`+trustServerCertificate=false+`).
** If the SQL Server certificate is not stored in the local Java TrustStore, the parameter `+;trustServerCertificate=true+` must be added to the connection string to allow the connection.
