...
Die Applikation SIARD Suite ist eine Java FX Applikation. Als solche kann sie grundsätzlich auf jedem System ausgeführt werden, auf welchem ein Java Runtime Environment (JRE) 17 mit der JavaFX Extension installiert ist.
...
Die Struktur innerhalb des spezifizierten Ordners wird anschliessend automatisch generiert und folgt der Struktur der Datenbank: `schema/table/column`. SIARD Suite detektiert automatisch den MIME-Type und die Dateiendungen werden automatisch gesetzt. Dies ermöglicht auch, dass die Binary-Daten direkt aus der SIARD Suite in der korrekten auf dem System installierten Applikation geöffnet werden können.
...
* *Java 17 Anforderungen:* Aus Sicherheitsgründen unterstützen aktuelle Java-Versionen die veralteten Protokolle TLS 1.0 und 1.1 nicht mehr standardmäßig. Es wird mindestens TLS 1.2 vorausgesetzt.
** _Hinweis:_ Falls für Altsysteme TLS 1.0/1.1 zwingend benötigt wird, muss dies explizit in der Datei `+java.security+` der JVM wieder aktiviert werden.
* *JDBC-Treiber (Verschlüsselung):* Ab Treiber-Version 10.2 (inkl. 13.4.0) ist die Verschlüsselung standardmäßig aktiv (`+encrypt=true+`).
* *Zertifikatsprüfung:* Der Treiber prüft nun streng, ob das Server-Zertifikat vertrauenswürdig ist (`+trustServerCertificate=false+`).
** Wenn das Zertifikat des SQL-Servers nicht im lokalen Java-Truststore hinterlegt ist, muss im Connection-String der Parameter `+;trustServerCertificate=true+` ergänzt werden, um die Verbindung zu ermöglichen.
