...
* Mit Java 17 neue Anforderungen, insbesondere wird TLS 1.2 nicht mehr unterstützt.
** TODO: falls nötig für Rückwärtskompatibilität beschreiben, wie für die JVM TLS 1.0 wieder aktiviert werden kann
* Wenn Verschlüsselung verwendet wird.
** Parameter `encrypt=true` setzen in der jdbc-url. Dann musste aber wohl das CA-Zertifikat im Truststore sein. Falls das nicht klappt, dann auch `trustServerCertificate=true` setzen, dann sollte die Verbindung klappen
