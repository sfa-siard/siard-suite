...
public class TestDb2Database {
    ...
    public static QualifiedId getQualifiedStructType() {
        return new QualifiedId(null, _sTEST_SCHEMA, _sTEST_TYPE_STRUCT);
    }
    ...
    private void insertSimpleTable()
        ...
        for (int iColumn = 0; iColumn < _listCdSimple.size(); iColumn++) {
            if (iColumn > 0)
                sbSql.append(",\r\n  ");
            sbSql.append(_listCdSimple.get(iColumn)
                                      .getName());
        }
        sbSql.append("\r\n)\r\nVALUES\r\n(\r\n  ");
        for (int iColumn = 0; iColumn < _listCdSimple.size(); iColumn++) {
        ...
        Reader rdrClob = new StringReader((String) _listCdSimple.get(3)
                                                                .getValue());
        pstmt.setCharacterStream(1, rdrClob);
        Reader rdrDbClob = new StringReader((String) _listCdSimple.get(6)
                                                                  .getValue());
        pstmt.setCharacterStream(2, rdrDbClob);
        InputStream isBlob = new ByteArrayInputStream((byte[]) _listCdSimple.get(10)
                                                                            .getValue());
    ...
    private void insertComplexTable()
        ...
        for (int iColumn = 0; iColumn < _listCdComplex.size(); iColumn++) {
            if (iColumn > 0)
                sbSql.append(",\r\n  ");
            sbSql.append(_listCdComplex.get(iColumn)
                                       .getName());
        }
        sbSql.append("\r\n)\r\nVALUES\r\n(\r\n  ");
        for (int iColumn = 0; iColumn < _listCdComplex.size(); iColumn++) {
        ...
        sbSql.append("\r\n)");
        PreparedStatement pstmt = _conn.prepareStatement(sbSql.toString());
        Reader rdrClob = new StringReader((String) _listAdStruct.get(2)
                                                                .getValue());
